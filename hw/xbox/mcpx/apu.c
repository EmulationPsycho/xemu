...
static void mcpx_apu_write(void *opaque, hwaddr addr, uint64_t val,
                           unsigned int size);
static void voice_off(MCPXAPUState *d, int v);
static void voice_lock(MCPXAPUState *d, int v, bool lock);
static bool is_voice_locked(MCPXAPUState *d, int v);
        ...
        for (int i = 0; d->regs[current] != 0xFFFF; i++) {
            ...
            if (!voice_get_mask(d, v, NV_PAVS_VOICE_PAR_STATE,
                ...
                if (!is_voice_locked(d, v)) {
                    /* FIXME: Should the entire engine stall? For now just
                     * skip locked voices */
                    qemu_spin_lock(&d->vp.voice_spinlocks[v]);
                    process_voice(d, mixbins, v);
                    qemu_spin_unlock(&d->vp.voice_spinlocks[v]);
                }
            }
